...
if [ -d "$HOME/swDev/javascript/lynda/node_modules/.bin" ] ; then
  # see if pathappend function exitst
  declare -F pathappend > /dev/null
  if [[ $? -eq 0 ]] ; then
    # pathappend exists, so use it
    pathappend "$HOME/swDev/javascript/lynda/node_modules/.bin"
  else
    # path append does not exits, so prepend manually
    PATH="$HOME/swDev/javascript/lynda/node_modules/.bin:$PATH"
  fi
fi
